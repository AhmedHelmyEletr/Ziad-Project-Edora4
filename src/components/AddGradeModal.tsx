...
import { X, GraduationCap, Users, MapPin } from 'lucide-react';
...
const AddGradeModal: React.FC<AddGradeModalProps> = ({ isOpen, onClose, teacherId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!gradeName.trim() || !groupName.trim() || !location.trim()) {
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4" dir="rtl">
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl w-full max-w-lg transform transition-all duration-300 ease-in-out animate-fade-in-up">
        <div className="flex justify-between items-center p-5 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800 tracking-tight">إضافة صف جديد</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="flex items-start mb-6">
            <div className="w-20 h-20 rounded-full bg-indigo-100 flex items-center justify-center mr-6 border-4 border-white shadow-md">
              <GraduationCap className="w-12 h-12 text-indigo-600" />
            </div>
            <div className="pt-2">
              <h3 className="text-2xl font-extrabold text-gray-900">إنشاء صف جديد</h3>
              <p className="text-indigo-600 text-lg font-medium">إضافة مجموعة صفية جديدة</p>
              <p className="text-gray-600 text-sm">املأ التفاصيل أدناه</p>
            </div>
          </div>
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <h4 className="font-semibold text-gray-500 text-sm uppercase tracking-wider mb-3">تفاصيل الصف</h4>
                ...
                <div className="bg-gray-100 p-4 rounded-lg">
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      className="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-right"
            ...
            <div className="flex justify-end space-x-3 mt-8">
              ...
              <button
                type="submit"
                className="px-6 py-3 bg-indigo-600 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
              >
                إضافة صف
              </button>
            </div>
